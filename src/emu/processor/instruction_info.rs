...
/// Cycle length depends on if opcode conditions were met
#[derive(Clone, Copy)]
pub struct CycleLength {
    /// Length if conditions were met
    pub full: u8,
    /// Length if conditions weren't met
    pub light: u8,
}
impl CycleLength {
...
/// Game Boy CPU instruction metadata
...
pub const MAIN_INST_INFO: [InstInfo; 256] = [
    ...
    InstInfo::new_s("LD (HL), L", 1, 2),    // 0x75
...
/// Game Boy CPU instruction metadata for opcodes prefixed by "CB"
